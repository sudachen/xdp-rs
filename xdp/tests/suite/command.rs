...
pub fn restart_with_caps(my_caps: &[caps::Capability]) -> Result<()>{
    ...
    log::info!("Current capabilities: {:?}", perm_caps);
    match env::var("RESTARTED") {
        Err(_) => {}
        Ok(val) => {
            if val == "1" {
                log::info!("Already restarted with capabilities, skipping.");
                return Ok(());
            }
        }
    }
    ...
    if my_caps.iter().any(|c| !perm_caps.contains(c)) {
        ...
        log::debug!("Re-executing with command: {:?} and args: {:?}", current_prog, args);
