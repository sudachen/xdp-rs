...
use std::{io, time};
use xdp_tests::xdp;
use xdp_util::{Router, get_ipv4_address, mac_by_ifindex};
...
pub fn main() -> io::Result<()> {
    ...
    let delay = args
        .delay
        .as_ref()
        .map_or(Ok(time::Duration::from_secs(1)), |x| {
            humantime::parse_duration(x).map_err(|_| io::Error::other("Invalid delay format"))
        })?;

    let src: SocketAddrV4 = args
        .src
        .parse()
        .map_err(|_| io::Error::other("Invalid source address format"))?;
    let dst: SocketAddrV4 = args
        .dst
        .parse()
        .map_err(|_| io::Error::other("Invalid source address format"))?;
    ...
    let _owned_xdp_host1 = xdp::attach_pass_program(if_index).map_err(|e| {
    ...
    let next_hop = router
        .route(dst.ip())
        .ok_or_else(|| io::Error::other(format!("No route to destination IP {}", dst.ip())))?;
    ...
    for _ in 0..args.count {
        let mut bf = sok
            .seek_and_peek(42 + bytes.len())
            .map_err(|e| io::Error::other(format!("Failed to seek and peek: {}", e)))?;
        ...
        sok.commit()
            .map_err(|e| io::Error::other(format!("Failed to commit buffer in RX ring: {e}")))?;
        sok.kick()?;
